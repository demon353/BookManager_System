...
public class BookController extends HttpServlet {
	...
	private void showBook(HttpServletRequest request, HttpServletResponse response) throws IOException {
		List<BookInfo> list = bis.showBook();
	...
	private void addBook(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if(isMultipart) {
			    ...
			    isPass=bis.addInfo(infos);
			    if(isPass) {
			    	//添加成功
			    	response.sendRedirect("BookController?op=showBook");
			    }else {
	...
	private void updateBook(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if(isMultipart) {
			    ...
			    isPass=bis.updateInfo(infos);
			    if(isPass) {
			    	//修改成功
			    	response.sendRedirect("BookController?op=showBook");
			    }else {
			    	response.sendRedirect("BookController?op=findById");
			    }
			}
		} catch (NumberFormatException e) {
			e.printStackTrace();
