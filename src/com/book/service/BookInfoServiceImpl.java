...
public class BookInfoServiceImpl implements BookInfoService {
	...
	@Override
	public BookInfo finById(int id) {
		sqlSession = MybatisUtils.createSqlSession();
		BookInfo bif =sqlSession.getMapper(BookMapper.class).finfById(id);
		if(bif != null) {
			return bif;
		}
		return null;
	...
	public boolean addInfo(BookInfo infos) {
		sqlSession = MybatisUtils.createSqlSession();
		int result = sqlSession.getMapper(BookMapper.class).addBook(infos);
		if (result>0) {
			sqlSession.commit();
			MybatisUtils.closeSqlSession(sqlSession);
			return true;
	...
	public List<BookInfo> showBook() {
		sqlSession = MybatisUtils.createSqlSession();
		List<BookInfo> list = sqlSession.getMapper(BookMapper.class).findBook();
		MybatisUtils.closeSqlSession(sqlSession);
		return list;
	}

	@Override
	public boolean updateInfo(BookInfo infos) {
		sqlSession = MybatisUtils.createSqlSession();
		int result = sqlSession.getMapper(BookMapper.class).updateBook(infos);
		if (result>0) {
			sqlSession.commit();
			MybatisUtils.closeSqlSession(sqlSession);
			return true;
